...
const CH = "hover:border-[#C9A84C]/40 hover:bg-[#151005]";
const CA = "border-[#C9A84C] bg-[#1a1308]";
const TO = "bg-[#C9A84C]/15 border-[#C9A84C]/50 text-[#C9A84C]";
const TF = "border-[#251c08] text-white/40 hover:border-[#C9A84C]/30 hover:text-white/70";
...
export default function BandBuilder({ onRequestQuote }: { onRequestQuote?: (d: any) => void }) {
    ...
    const INPUT = "w-full bg-[#0d0a05] border border-[#251c08] rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-[#C9A84C]/50 transition-colors placeholder:text-white/20 appearance-none [color-scheme:dark]";
    const LABEL = "text-[9px] uppercase tracking-widest text-[#C9A84C]/40 mb-1 block";
        ...
        switch (step) {
                ...
                <div className="grid grid-cols-2 gap-3">
                    {EVENT_TYPES.map(t => (
                        <button key={t} onClick={() => { setEventType(e => e===t?"":t); }}
                            className={`px-4 py-4 rounded-lg border text-left text-sm font-medium transition-all duration-150 active:scale-[0.98] flex items-center justify-between group
                                ${eventType===t ? CA : `${CB} ${CC} ${CH} text-white/60`}`}>
                            <span className={eventType===t ? "text-[#C9A84C]" : ""}>{t}</span>
                    ...
                    {MUSIC_STYLES.map(s => {
                        ...
                        return (
                            <button key={s.id} onClick={() => toggleStyle(s.id)}
                                className={`px-4 py-4 rounded-lg border text-left text-sm transition-all duration-150 active:scale-[0.98] flex items-center justify-between
                                    ${on ? CA : `${CB} ${CC} ${CH} text-white/60`}`}>
                                <div className="flex items-center gap-2.5">
                                    <Music className={`w-4 h-4 shrink-0 ${on ? "text-[#C9A84C]" : "text-[#C9A84C]/25"}`} />
                                    <span className={`font-medium ${on ? "text-[#C9A84C]" : ""}`}>{s.label}</span>
                                </div>
            ...
            case 3: return selectedStyles.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                    <Music className="w-10 h-10 text-[#C9A84C]/20" />
                    <p className="text-white/30 text-sm">Go back and select at least one music vibe first.</p>
                    <button onClick={prev} className="text-[#C9A84C] text-xs underline">← Pick a Vibe</button>
                </div>
            ) : (
                    ...
                    {selectedStyles.map(style => {
                        ...
                        return (
                            <div key={style.id} className={`${CB} border ${CC} rounded-xl p-4`}>
                                <p className="text-[#C9A84C] text-xs font-semibold flex items-center gap-1.5 mb-3">
                                    <Sparkles className="w-3 h-3" /> {style.label}
                                </p>
                                <div className="mb-3">
                                    <p className="text-[9px] uppercase tracking-widest text-white/25 mb-2 flex items-center gap-1"><Mic2 className="w-2.5 h-2.5" /> Vocals</p>
                                    <div className="flex flex-wrap gap-1.5">
                                        {config.vocals.map(v => <Tag key={v} label={v} on={userConf.vocals.includes(v)} onClick={() => updateVibe(style.id,"vocals",v)} />)}
                                    </div>
                                </div>
                                <div>
                                    <p className="text-[9px] uppercase tracking-widest text-white/25 mb-2 flex items-center gap-1"><Guitar className="w-2.5 h-2.5" /> Instruments</p>
                                    <div className="flex flex-wrap gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                    {BAND_SIZES.map(size => (
                        <button key={size} onClick={() => setBandSize(s => s===size?"":size)}
                            ...
                            <Users className={`w-6 h-6 ${bandSize===size ? "text-[#C9A84C]" : "text-[#C9A84C]/25"}`} />
                            <span className={`text-sm font-medium ${bandSize===size ? "text-[#C9A84C]" : "text-white/60"}`}>{size}</span>
                        </button>
                    ))}
                </div>
            );
            ...
            case 6: return (
                <div className="space-y-3 overflow-y-auto scrollbar-hide max-h-full">
                    ...
                    <button onClick={()=>setModal(true)}
                        className="w-full py-2 border border-[#251c08] text-white/30 text-xs rounded-full hover:text-white/50 hover:bg-[#151005] transition-colors">
        ...
        <div className="w-full h-[90dvh] md:h-[90vh] flex items-stretch bg-[#080602] overflow-hidden rounded-2xl relative">
            ...
            <div className="hidden md:flex w-[42%] shrink-0 relative flex-col overflow-hidden bg-[#0d0a05] z-10">
                ...
                <div className="relative z-10 p-8">
                    <p className="text-[#C9A84C]/70 text-[10px] uppercase tracking-[0.3em]">The Band Company</p>
                </div>

                {/* Step label */}
                <div className="relative z-10 mt-auto p-8">
                    <p className="text-[#C9A84C]/50 text-[10px] uppercase tracking-widest mb-2">{stepMeta.num} / 06</p>
                    <h2 className="text-3xl font-serif text-white leading-tight">
                        Craft Your<br /><span className="text-[#C9A84C]">Dream Band</span>
                    </h2>
                    <p className="text-white/30 text-xs mt-2 max-w-[200px]">Design a live music experience as unique as your celebration.</p>
            ...
            <div className="flex-1 flex flex-col bg-transparent md:bg-[#0a0703] overflow-hidden z-10 relative">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-[#1a1408] bg-[#0a0703]/60 backdrop-blur-md md:bg-transparent md:backdrop-blur-none shrink-0 relative z-20">
                    <div>
                        <p className="text-[#C9A84C] text-xs font-semibold tracking-wider">{stepMeta.num} — {stepMeta.label}</p>
                        <p className="text-white/25 text-[10px] mt-0.5">Step {step} of 6</p>
                    </div>
                    {/* Step tabs — compact */}
                                ...
                                className={`w-7 h-7 rounded-full text-[10px] font-bold transition-all duration-200
                                    ${step===i+1 ? "bg-[#C9A84C] text-black" :
                                      step>i+1  ? "bg-[#C9A84C]/20 text-[#C9A84C] border border-[#C9A84C]/40" :
                                                   "bg-[#1a1408] text-white/20 border border-[#251c08]"}`}>
                                {step > i+1 ? "✓" : s.num}
                            </button>
                        ))}
                    </div>
                ...
                <div className="shrink-0 px-6 py-4 border-t border-[#1a1408] bg-[#0a0703]/80 backdrop-blur-md md:bg-transparent md:backdrop-blur-none flex items-center justify-between gap-4 relative z-20">
                    <button onClick={prev} disabled={step===1}
                        className="flex items-center gap-2 text-sm text-[#C9A84C]/50 hover:text-[#C9A84C] active:scale-95 disabled:active:scale-100 disabled:opacity-20 disabled:cursor-not-allowed transition-all">
            ...
            {modal && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4"
                    ...
                    <div className="bg-[#0d0a05] border border-[#251c08] rounded-2xl p-8 max-w-sm w-full"
                        ...
                        <p className="text-white/30 text-xs mb-5">How would you like to connect?</p>
                        <div className="space-y-2">
                            <a href="https://wa.me/7779945379?text=Hi!%20I%20need%20expert%20recommendation." target="_blank" rel="noopener noreferrer"
                                className="flex items-center justify-center gap-2 w-full py-3 bg-[#25D366] text-white text-sm font-medium rounded-full hover:bg-[#20BA5A] transition-colors">
                                <svg viewBox="0 0 24 24" className="w-4 h-4 fill-white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
                                WhatsApp Chat
                            </a>
                            <a href="tel:7779945379"
                                className="flex items-center justify-center gap-2 w-full py-3 bg-blue-600 text-white text-sm font-medium rounded-full hover:bg-blue-500 transition-colors">
                                <Phone className="w-4 h-4" /> Call Us
                            </a>
                            <button onClick={() => setModal(false)}
                                className="w-full py-2.5 text-white/25 hover:text-white/50 text-xs transition-colors">
                                Cancel
                            </button>
                        </div>
                    </div>
...
function Row({ label, value }: { label: string; value?: string }) {
    ...
    return (
        <div className="flex items-start justify-between gap-4">
            <p className="text-[10px] uppercase tracking-widest text-[#C9A84C]/40 shrink-0">{label}</p>
